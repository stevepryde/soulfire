...
| Phase 4: Tauri bridge | Partial | `src-tauri` now provides the Tauri v2 crate/config/capability scaffold, narrow app command permissions, async typed setup/unlock/profile/settings/credential commands, token-stats read/clear commands, image load/generate/upload/clear commands, character prompt-view/save-section commands, adventure prompt-view commands, character save/list/load/delete/count, character/world builder state/send/undo commands, NPC extraction commands, chat load/delete/open/send, world save/list/load/delete/count, adventure list/load/delete/start/turn/GM-proposal decision commands, and the first event DTO vocabulary. |
| Phase 5: React UI fidelity port | Partial | Vite/React/Tailwind/Bun scaffold exists with local setup/unlock, shell navigation, Tauri-backed Worlds/Characters read surfaces with total chips, and OpenAI credential status/save/delete in Settings. OG detail/editor/play screens remain reference-only until the fidelity port lands. |
...
| Phase 7: validation | Partial | Core tests exist; OG prompt/config fixture parity, service-flow parity, Tauri command checks, a first React shell smoke, and manual smoke docs exist. Broader React visual/app-journey smoke remains. |
...
| OG source | Local source | Status | Notes |
| --- | --- | --- | --- |
| World blueprint create/read/update/delete/list/count | `model/world.rs`, `store/repo/worlds.rs`, `src-tauri/src/commands/worlds.rs` | Partial bridge | Core model and repository exist. Tauri exposes save, cursor list, load, delete, and count commands; React shell shows blueprint totals in the list header. Full create/editor UI remains missing. |
...
| Worlds home | Partial data-backed shell | React loads in-progress adventures plus debounced searchable, cursor-paged world blueprints through Tauri commands with OG-style tabs/search chrome, 16:6 placeholder cards, loading/error/empty states, read-only blueprint/adventure state and message detail panels, adventure-scoped token stats, and in-app delete confirmations. Port generated cover art, card actions, create, and full editor/play entry behavior, minus public/admin tabs. |
...
| Character list | Partial data-backed shell | React loads/debounced-searches cursor-paged saved characters through Tauri commands with OG-style search chrome, stacked avatar rows, loading/error/empty states, total count chip, read-only character detail panels, and in-app delete confirmation. Port OG portrait art, create, editor, and chat entry behavior. |
...
| Settings/profile | Partial data-backed shell | Store status/schema/runtime, lock-store action, app profile editing with custom language and default-model buttons, OpenAI credential status/save/delete with in-app delete confirmation, accent swatches, adult-content toggle editing, and player profile editing exist. Port remaining local-only storage actions and OG settings affordances. |
| Token stats | Partial data-backed shell | Tauri exposes aggregate, per-chat, per-adventure, and clear-history token stats commands built from local metrics. React exposes aggregate totals, model/operation breakdowns, adventure-scoped totals from active adventure detail, and in-app clear-history confirmation; scoped chat stats remain. |
| Modals/toasts/confirmation/error/loading/empty states | Partial | Loading/error/empty states exist on shell panels, and token-history, OpenAI key deletion, plus shell character/world/adventure deletion use in-app confirmation modals. Port remaining OG modal/toast patterns. |
...
| Tauri command/event tests | Partial | `soulfire-app` has async state-boundary tests, including lock-cycle persistence through the AsyncStore facade, plus event serialization tests for React-facing names/fields, and core observed-progress tests for chat/adventure echoes. Command registration and permission schemas are generated by the app crate checks; add direct command invocation tests as feature commands/events land. |
| React smoke/visual tests | Partial | `src/App.smoke.test.tsx` exercises the unlocked shell and mocked Tauri data-backed Worlds, Characters, Stats, and Settings panels. Add native/webview visual smoke and full chat/play journey checks as those surfaces land. |
