...
| Worlds home | Partial data-backed shell | React loads in-progress adventures plus searchable, cursor-paged world blueprints through Tauri commands with loading/error/empty states, read-only blueprint/adventure detail panels, adventure-scoped token stats, and in-app delete confirmations. Port OG card richness, create, and full editor/play entry behavior, minus public/admin tabs. |
