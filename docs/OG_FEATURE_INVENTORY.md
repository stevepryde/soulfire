...
| Settings/profile | Partial data-backed shell | Store status/schema/runtime, lock-store action, app profile editing with custom language buttons, OpenAI credential status/save/delete with in-app delete confirmation, accent swatches, adult-content toggle editing, and player profile editing exist. Port remaining local-only storage actions and OG settings affordances. |
