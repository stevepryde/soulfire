...
| Worlds home | Partial data-backed shell | React loads in-progress adventures plus searchable, cursor-paged world blueprints through Tauri commands with loading/error/empty states, read-only blueprint/adventure state and message detail panels, adventure-scoped token stats, and in-app delete confirmations. Port OG card richness, create, and full editor/play entry behavior, minus public/admin tabs. |
...
| Character list | Partial data-backed shell | React loads/searches cursor-paged saved characters through Tauri commands with loading/error/empty states, total count chip, read-only character detail panels, and in-app delete confirmation. Port OG card art, create, editor, and chat entry behavior. |
