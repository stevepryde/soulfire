...
impl WorldEngine {
    ...
    pub async fn start_adventure<F: FnMut(&str)>(
        ...
        self.meter(MetricLabel::AdventureAction, model, intro.usage, None, Some(&blueprint.blueprint_id))?;
        ...
        let adventure = Adventure::builder()
            ...
            .maybe_player_name((!player.player_name.as_str().is_empty()).then(|| player.player_name.clone()))
            .maybe_player_attributes(
                (!player.player_attributes.as_str().is_empty()).then(|| player.player_attributes.clone()),
            )
            .ai_model(model)
        ...
        let intro_msg = self.new_message(&adventure.adventure_id, AdventureMessageType::Narration, &intro.text);
    ...
    pub async fn take_turn<F: FnMut(&str)>(
        ...
        match parse_turn_input(raw_input) {
            TurnInput::Action(action) => self.run_action_turn(adventure_id, &action, on_delta).await,
            ...
            TurnInput::Unknown(cmd) => {
                Ok(TurnOutcome::Warning(format!("Unknown command: {cmd}")))
            }
        }
    }
    ...
    async fn run_action_turn<F: FnMut(&str)>(
        ...
        let player_ext = self.store.player_profile()?.prompt_extension.map(|p| p.to_string());
        ...
        let narration_msg =
            self.new_message(adventure_id, AdventureMessageType::Narration, &narration.text);
        self.store.save_adventure_message(&narration_msg)?;
        self.meter(MetricLabel::AdventureAction, model, narration.usage, Some(adventure_id), None)?;
    ...
    async fn reconcile_state(
        ...
        let compaction = if adventure.adventure_state.as_str().len() > LARGE_STATE_COMPACTION_THRESHOLD {
        ...
        if !force_full {
            let instructions =
                prompts::diff_state_update_instructions(adventure.world_prompt.as_str(), compaction, adult);
            if let Ok(resp) = self
                .state_call(model, instructions, input.clone(), MetricLabel::AdventureDiffStateUpdate, adventure)
                .await
            {
        ...
        let instructions =
            prompts::full_state_update_instructions(adventure.world_prompt.as_str(), compaction, adult);
        let resp = self
            .state_call(model, instructions, input, MetricLabel::AdventureFullStateUpdate, adventure)
            .await?;
    ...
    /// guard (`WORLD-10`).
    fn apply_diff_memory(&self, adventure: &mut Adventure, new_state: &Value, diff: &response::DiffUpdate) {
        adventure.adventure_state = AdventureState::coerce(&serde_json::to_string(new_state).unwrap_or_default());
        ...
        let (mut updated, _next) =
            memory::apply_significant_event_updates(&existing_sig, &diff.significant_event_updates, next_id);
    ...
    fn set_significant(&self, adventure: &mut Adventure, new: Vec<SignificantEvent>, prior: &[SignificantEvent]) {
        if new.is_empty() && !prior.is_empty() {
            return; // no-wipe guard
        }
        adventure.next_significant_event_id = next_id_from_events(&new);
        adventure.significant_events = SignificantEvents::coerce(&serialize_significant_events(&new));
    }
    ...
    async fn state_call(
        ...
        self.meter(label, model, resp.usage, Some(&adventure.adventure_id), None)?;
        Ok(resp)
    }
    ...
    async fn run_gm_request(
        ...
        let req_msg = self.new_message(adventure_id, AdventureMessageType::GameMasterRequest, request);
        self.store.save_adventure_message(&req_msg)?;

        let model = adventure.ai_model.unwrap_or_else(AiModel::default_chat_narrative);
        let adult = self.store.app_settings()?.content_toggles.adult_content;
        ...
        let classify = self
            ...
            .await?;
        self.meter(MetricLabel::GmCommand, model, classify.usage, Some(adventure_id), None)?;
        ...
        if intent == GmIntent::AnswerOnly {
            let resp = self
                ...
                .generate(GenerationRequest {
                    model,
                    instructions: Some(prompts::gm_answer_instructions(adventure.world_prompt.as_str())),
                    messages: input,
                    config: GenerationConfig { json: Some(JsonMode::Json), ..Default::default() },
                })
                .await?;
            self.meter(MetricLabel::GmCommand, model, resp.usage, Some(adventure_id), None)?;
            let answer = response::parse_gm_answer(&resp.text);
            let msg = self.new_message(adventure_id, AdventureMessageType::GameMasterResponse, &answer);
        ...
        let proposal = GmProposal::builder()
            ...
            .maybe_proposed_adventure_state(
                proposal_data.updated_adventure_state.as_deref().map(AdventureState::coerce),
            )
            .maybe_proposed_world_prompt(
                proposal_data.updated_world_blueprint.as_deref().map(WorldPrompt::coerce),
            )
            .changes(changes)
            .build();
        ...
        adventure.ready_status = AdventureReadyStatus::Ready;
        self.store.save_adventure(&adventure)?;
        Ok(TurnOutcome::GmProposal { message: msg, proposal })
    }
    ...
    pub async fn accept_proposal(&self, proposal_id: &lib_soulfire::ids::GmProposalId) -> CoreResult<()> {
    ...
    pub async fn reject_proposal(&self, proposal_id: &lib_soulfire::ids::GmProposalId) -> CoreResult<()> {
        let mut proposal = self
            .store
            .gm_proposal(proposal_id)?
            .ok_or_else(|| CoreError::NotFound(proposal_id.to_string()))?;
    ...
    fn claim_lock(&self, adventure: &mut Adventure, status: AdventureReadyStatus) -> CoreResult<()> {
        let now = self.clock.now();
    ...
    fn compute_diff(&self, adventure: &Adventure, proposal: &response::GmProposalResponse) -> Vec<GmDiffEntry> {
        let mut entries = Vec::new();
        if let Some(new_state) = &proposal.updated_adventure_state {
            let before: Value = serde_json::from_str(adventure.adventure_state.as_str())
